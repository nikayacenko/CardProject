// apps/cardproject/src/test/java/com/example/cardproject/ui/deck/DeckListViewModelIntegrationTest.kt
...
class DeckListViewModelIntegrationTest {
    ...
    private fun injectRepository(viewModel: DeckListViewModel, repository: DeckRepository) {
        try {
            ...
            println("⚠️ Не удалось внедрить репозиторий через рефлексию: ${e.message}")
        }
    }
    ...
    fun deckCardCount_shouldUpdateWhenCardsAreAdded() = runTest(testDispatcher) {
        ...
        // Проверяем, что колода существует
        val deckWithCount = viewModel.decks.value.find { it.deck.id == deckId }
        assertNotNull("Колода должна существовать", deckWithCount)

        // Проверяем начальное количество карточек (0)
        val initialCards = cardRepository.getCardsByDeckSync(deckId)
        assertEquals("Изначально карточек должно быть 0", 0, initialCards.size)

        // when: добавляем карточку в колоду
        val card = com.example.cardproject.model.Card(
            deckId = deckId,
            front = "Что такое Kotlin?",
            back = "Современный язык програмлирования"
        )
        cardDao.insertCard(card)
        advanceUntilIdle()

        // then: количество карточек должно стать 1
        viewModel.refreshData()
        advanceUntilIdle()
