...
class DeckDaoTest {
    ...
    @Before
    fun setup() {
        // Создание тестовой базы данных в памяти (не сохраняется на диск)
     ...
     * Означает: обязательные поля корректно обрабатываются, необязательные могут быть null
     */
    @Test
    fun createDeck_withMinimalData_shouldUseNullForOptionalFields() = runBlocking {
        // given
        ...
        assertNull("Описание должно быть null", savedDeck?.description)
        assertNull("Цвет должен быть null", savedDeck?.coverColor)
    }

    /**
     * Проверяет: создание колоды без имени (обязательное поле)
     * Ожидает: ошибка целостности данных
     */
    @Test(expected = Exception::class)
    fun createDeck_withoutName_shouldThrowException(): Unit = runBlocking {
        // given: колода без обязательного поля name
        val deck = Deck(name = "")

        // when: попытка сохранения
        deckDao.insertDeck(deck)

        // then: должно выбросить исключение (тест ожидает Exception)
    }
    ...
    /**
     * Проверяет: получение всех колод
     * Ожидает: возвращаются все колоды
     */
    @Test
    fun getAllDecks_withMultipleDecks_shouldReturnAll() = runBlocking {
        // given
        deckDao.insertDeck(Deck(name = "Первая колода"))
        deckDao.insertDeck(Deck(name = "Вторая колода"))
        deckDao.insertDeck(Deck(name = "Третья колода"))

        // when
        val allDecks = deckDao.getAllDecks().toList()

        // then
        assertEquals("Должны быть получены все 3 колоды", 3, allDecks.size)
        // Проверяем только количество, не трогаем id
        assertTrue(allDecks.isNotEmpty())
    }
    ...
    fun deleteDeck_byDeckObject_shouldRemoveFromDatabase() = runBlocking {
        // given
        val deck = Deck(name = "Колода для удаления")
        val deckId = deckDao.insertDeck(deck)
        ...
        assertNull("Колода должна быть удалена", deletedDeck)
    }
    ...
    fun deleteDeck_withCards_shouldCascadeDeleteCards() = runBlocking {
        ...
        val cards = cardDao.getCardsByDeck(deckId).first()  // корутины
        assertEquals(2, cards.size)
        // when: удаление колоды
        deckDao.deleteDeck(deck)
